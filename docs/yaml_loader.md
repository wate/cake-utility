...
- 解決タイミング: `resolve()` メソッドの呼び出し時に、リファレンスマップに基づいて実際のIDに置換されます。
...
- 処理: `parse()` フェーズで解決され、`resolve()` フェーズにてDBスキーマ (Date, Time, DateTime) に合わせた型変換が行われます。
...
- 値が数値として解釈されるのを避けたい場合、ダブルクォート (`"00123"`) で囲むことで文字列として扱います。
