...
現在のパージ時CSV出力は `AuditLogPurgeService` 内で直接ファイル書き込みを行っています。
将来、汎用的なエクスポート機能をプラグインに追加する場合、パージ時CSV出力もその基盤を利用する形に見直せるかどうかを併せて検討してください。監査ログ以外のエクスポート処理と出力形式を統一できる可能性がありますが、必須ではありません。
